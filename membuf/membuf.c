...
#define MEMBUF_IOCTL_MAGIC 'm'
#define MEMBUF_GET_SIZE _IOR(MEMBUF_IOCTL_MAGIC, 1, int)
#define MEMBUF_SET_SIZE _IOW(MEMBUF_IOCTL_MAGIC, 2, int)
...
static int set_num_devices(const char *val, const struct kernel_param *kp) {
    ...
    if (ret) return ret;
    if (new_count < 0 || new_count > MAX_DEVICES) return -EINVAL;
    ...
    switch (cmd) {
        ...
        case MEMBUF_SET_SIZE:
...

            {
                ...
                size_t copy_len = min(dev->size, (size_t)new_size);
...
static int __init membuf_init(void) {
    ...
    for (i = 0; i < num_devices; i++) {
        ret = create_membuf_device(i);
        if (ret < 0) goto err_remove_cdevs;
    }
...
err_remove_cdevs:
    while (i--) destroy_membuf_device(i);
